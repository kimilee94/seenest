...
Seenest 从产品设计上面向多个内容平台。当前版本已支持 **X / Twitter 帖子与文章**、**哔哩哔哩视频详情页**，以及 **GitHub 公开仓库与 Issue**。B 站和 GitHub 适配器都只会在用户主动开启后请求站点权限。
...
- 收好 GitHub 公开仓库的简介、主题、语言、Star、Fork 与最多 1000 字符的 README 核心片段
- 收好 GitHub 公开 Issue 的首帖正文，最多 500 字符，不抓评论和私有仓库
...
| GitHub | 已支持，需主动开启 | 公开仓库首页与单条 Issue |
...
X 使用单页应用路由，因此 Seenest 会进行轻量的地址检查。DOM 观察器只在短时采集会话中运行，并在采集成功、超时或切换页面后停止。B 站从已打开的视频地址读取 BVID，再不带 Cookie 地请求一次公开视频详情接口。GitHub 会在已聚焦的公开页面稳定后解析当前 DOM，不调用 GitHub API，也不扫描私有仓库。
...
6. 打开一个已支持的 X 详情页。如需收好 B 站或 GitHub 内容，请在 Seenest 的“网站权限”中开启对应来源，确认可选站点授权后再访问支持的详情页。
