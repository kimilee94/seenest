...
Seenest 收好的内容、访问次数和活跃停留累计保存在 IndexedDB。活跃停留只根据页面可见性、窗口焦点、近期交互和系统空闲状态在本机推算，不保存按键内容、鼠标坐标或滚动内容。只有用户主动选择并授权一个本地 JSON 文件后，Seenest 才会写入自动快照。卸载扩展可能同时删除浏览器管理的本地数据库，如果这些内容很重要，请先导出或开启备份。
